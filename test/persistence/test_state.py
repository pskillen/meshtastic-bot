...
class TestFileBasedStatePersistence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.state = AppState(
            packet_counter_reset_time=datetime(2023, 1, 1, 12, 0, 0),
            command_stats={'user1': {'command1': 1}},
            unknown_command_stats={'user1': {'unknown_command1': 1}}
        )
    ...
    def test_persist_state(self, mock_json_dump, mock_file):
        ...
        mock_json_dump.assert_called_once_with(
            {
                'packet_counter_reset_time': '2023-01-01T12:00:00',
                'command_stats': {'user1': {'command1': 1}},
                'unknown_command_stats': {'user1': {'unknown_command1': 1}}
            },
            mock_file()
    ...
    @patch('builtins.open',
           ...
           read_data='{"packet_counter_reset_time": "2023-01-01T12:00:00", "command_stats": {"user1": {"command1": 1}}, "unknown_command_stats": {"user1": {"unknown_command1": 1}}}')
    def test_load_state(self, mock_file):
        state = self.persistence.load_state()
        self.assertIsNotNone(state)
        self.assertEqual(state.packet_counter_reset_time, datetime(2023, 1, 1, 12, 0, 0))
        self.assertEqual(state.command_stats, {'user1': {'command1': 1}})
        self.assertEqual(state.unknown_command_stats, {'user1': {'unknown_command1': 1}})
